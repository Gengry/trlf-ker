...
skb->transport_header = (struct tcphdr*)skb_put(skb, sizeof (struct tcphdr));
...
cp_dev_xmit_tcp (ETH, SMAC, DMAC,NULL, 0, in_aton(SIP),in_aton(DIP),htons(SPORT),htons(DPORT),0,0,0,0);
	return 0;
}
